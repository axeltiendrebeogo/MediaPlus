...
export default function ControleurAffectations() {
  ...
  useEffect(() => {
    if (USE_MOCK) {
      ...
      // init selections sur les valeurs actuelles
      const init = {};
    ...
    // else: Promise.all([api.get('/medias/'), api.get('/users/?role=gestionnaire')]).then(...)
  }, []);
  ...
  const doAffectation = async (mediaId, gestId, gestNom) => {
    ...
    try {
      if (!USE_MOCK) {
        await api.patch(`/medias/${mediaId}/affecter/`, { gestionnaire_id: gestId });
      }
      // Mise à jour locale
      setData(prev => ({
        ...prev,
